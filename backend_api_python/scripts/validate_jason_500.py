"""Real-data validation gate for Jason US Swing $1,000.
...
def monte_carlo(r,paths=5000,seed=1000):
...
def main():
    ...
    ok=all(gates.values()); payload={'strategy':'Jason US Swing 1000','mode':'research_only','capital_usd':CFG.capital,'margin':False,'leverage':False,'holding_days':'1-10','preferred_efficiency_days':'1-5','broker_connected':False,'live_orders':False,'data_mode':'point_in_time_real','synthetic_allowed':False,'qualification':'PASS' if ok else 'REJECT','hard_gates_passed':ok,'trade_decision':'RESEARCH_CANDIDATES_ONLY' if ok else 'NO_TRADE','today':'仅研究候选，不自动交易' if ok else '今天不交易','independent_test':m,'walk_forward':folds,'vectorbt':metrics(vb),'event_driven':metrics(q),'engine_total_return_delta':delta,'monte_carlo':mc,'gates':gates}
